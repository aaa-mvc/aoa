...
[[AOA 项目演进全纪录 6.6.23 2011]]
[[AOA 项目演进全纪录 6.6.23 2021]]
[[aoa 终端命令行演变过程]]



从一份 8 行 JSON 配置文件到一个完整的 Agent 审计引擎。
...
python cli.py run claude-agent --audience boss   # 老板视角：只看成本/结论
```
...
|命令|场景|
|---|---|
|`python cli.py interactive`|双击即用，数字选单|
|`python cli.py run <profile>`|Human 审计（desktop/code/brain）|
|`python cli.py run claude-agent`|Agent 审计|
|`python cli.py discover agents`|本机 Agent 发现|
|`python cli.py aggregate feedback`|管理者汇总反馈|